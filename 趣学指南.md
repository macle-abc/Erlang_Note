...
2. 原子
   ...
   **原子引用到原子表中，不会被GC掉，64位系统每个原子占据8字节**
...
6. 二进制数据
   ...
   ### 关于TypeSpecifierList的说明（由一下一个或多个组成，使用-分隔）
...
```erlang shell
c() 
compile:file(FileName) % 2> compile:file(unless).    
cd(Path). % 用于切换工作目录 
```
...
gb_trees模块(通用平衡树)(存在两种模式，一种是简单模式不知道自己的数据在树中的存在情况，一种是智能模式默认知晓keyvalue在书中的存在情况此时会绕过所有安全检查)
...
该信号无法被捕获(process_flag(trap_exit, true)无法捕获)，erlang为了避免kill掉一个进程导致其他关联的进程全部被kill，于是传递给其他进程的退出原因改成了killed，但是对于被kill掉的进程接受到的信号实际上是kill
